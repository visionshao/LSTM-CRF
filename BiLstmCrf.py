...
# data = torch.tensor([[1, 2, 3, 4], [6, 0, 0, 0], [4, 5, 4, 0]])
# y = torch.tensor([[1, 0, 0, 1], [1, 0, 0, 1], [1, 1, 0, 0]])
# length = torch.tensor([4, 1, 3])
# mask = get_mask(length)
# print(mask)
# model = LstmCrf(10, 10, 4, "lstm", 100, 3, 4)
# out = model.forward(input_data=data, input_len=length)
# total_score = model.get_loss(input_data=data, input_len=length, targets=y, mask=mask)
# path = model.get_best(out, mask)
